"""
Основной сервис для проверки конфликтов прав

Реализует логику валидации запросов на получение прав пользователями
с учетом конфликтов между группами.
"""
...
class ValidationService:
    ...
    async def validate(self, request: ValidationRequest) -> ValidationResult:
        ...
        Логика:
        1. Получить текущие активные группы пользователя
        2. Определить новые группы для проверки:
           - Если permission_type == 'group' -> new_groups = [item_id]
           - Если permission_type == 'access' -> получить группы, содержащие этот доступ
        3. Получить матрицу конфликтов
        4. Проверить конфликты:
           - Для каждой конфликтующей пары (group1, group2):
             * Если у пользователя есть group1, а запрашивается group2 -> ОТКЛОНЕНО
             * Если у пользователя есть group2, а запрашивается group1 -> ОТКЛОНЕНО
        5. Вернуть результат
        """
        try:
            # 1. Получить текущие активные группы пользователя
            ...
            # Если нет новых групп для проверки (например, доступ не найден)
            if not new_groups:
            ...
            # Если не удалось получить данные - отклонить заявку
    ...
    async def _get_new_groups_for_validation(
        ...
        if permission_type == "group":
            ...
            # Для доступа - получаем группы, содержащие этот доступ
    ...
    async def _check_conflicts(
        ...
        Логика:
        - Для каждой конфликтующей пары (group1, group2) из матрицы:
        ...
        """
        # Если у пользователя нет групп или нет новых групп - конфликтов нет
        ...
        # Проверяем каждую конфликтующую пару
